...
{
	if constexpr (HasCustomSerialization<T>::value)
	{
		return HasCustomSerialization<T>::Serialize(*this, object, name);
	}
...
bool DataFile::Serialize_Basic(const T& object, const char* name)
{
	...
	if (mParserXml.CreateNode(name))
	{
		...
		{
			const std::string enumValueStr(Enum::GetValueName(object));
...
bool DataFile::Deserialize_Common(T& object, const char* name)
{
	if constexpr (HasCustomSerialization<T>::value)
	{
		return HasCustomSerialization<T>::Deserialize(*this, object, name);
	}
	else if constexpr (Meta::IsRegistered<T>())
	{
