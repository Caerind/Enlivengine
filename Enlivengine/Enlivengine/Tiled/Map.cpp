...
bool Map::LoadFromFile(const std::string& filename)
{
	...
	if (xml.ReadNode("map"))
	{
		...
		if (attribStr.size() > 0)
		{
			if (attribStr[0] == '#')
			{
				mBackgroundColor.FromRGBA(FromString<U32>(attribStr.substr(1)));
			}
			else
			{
				mBackgroundColor.FromRGBA(FromString<U32>(attribStr));
			}
		}
...
void Map::Render(bool renderObjects /*= false*/) const
{
    for (const LayerBase::Ptr& layer : mLayers)
    {
        if (layer != nullptr)
