...
public:
    ...
    template<typename... Component>
    auto create() {
        entity_type entitiesT[1]{};

        if constexpr(sizeof...(Component) == 0) {
            create<Component...>(std::begin(entitiesT), std::end(entitiesT));
            return entitiesT[0];
        } else {
            auto it = create<Component...>(std::begin(entitiesT), std::end(entitiesT));
            return std::tuple<entity_type, decltype(assign<Component>(entitiesT[0]))...>{entitiesT[0], *std::get<typename pool_type<Component>::iterator_type>(it)...};
        }
    }
