...
int Engine::Main(int argc, char** argv)
{
	...
	if (Engine::Init(argc, argv))
	{
		while (Engine::Update())
		{
...
#ifdef ENLIVE_TOOL
            RenderTarget& renderTarget = ImGuiEditor::GetRenderTarget();
#else
            RenderTarget& renderTarget = Framebuffer::GetDefaultFramebuffer();
#endif // ENLIVE_TOOL

			enAssert(renderTarget.IsValid());
            renderTarget.Clear(Colors::DarkGray);
			...
			GetInstance().mWindow.GetSFMLWindow().display(); // TODO : Clean ?
		}
	}
	Engine::Release();
	return 0;
...
bool Engine::Init(int argc, char** argv)
{
	...
	enLogInfo(LogChannel::Global, "ExecutablePath: {}", PathManager::GetExecutablePath().c_str());
	enLogInfo(LogChannel::Global, "CurrentPath: {}", PathManager::GetCurrentPath().c_str());
	...
	enLogInfo(LogChannel::Global, "AssetsPath: {}", PathManager::GetAssetsPath().c_str());
	...
	enLogInfo(LogChannel::Global, "TmpPath: {}", PathManager::GetTmpPath().c_str());

	AudioManager::GetInstance().Initialize();

	const bool windowCreated = engine.mWindow.Create("Enlivengine");
	if (!windowCreated)
	{
...
#endif // ENLIVE_TOOL
	if (!ImGuiWrapper::Init(engine.mWindow, imGuiIniFilePath))
	{
		return false;
	}
...
bool Engine::Update()
{
	...
	if (!engine.mWindow.ShouldClose())
	{
		...
		EventSystem::Update(engine.mWindow);
