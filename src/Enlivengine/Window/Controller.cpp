...
{
    return 0; // TODO static_cast<U32>(SDL_NumJoysticks());
}

U32 Controller::GetHapticCount()
{
    return 0; // TODO static_cast<U32>(SDL_NumHaptics());
}
...
bool Controller::Rumble(U32 controllerId, F32 strength, Time duration)
{
    ENLIVE_UNUSED(controllerId);
    ENLIVE_UNUSED(strength);
    ENLIVE_UNUSED(duration);
    // TODO
    /*
    if (Joystick* joystick = GetJoystickFromControllerID(controllerId))
    {
        ...
        {
            return SDL_HapticRumblePlay(haptic, strength, duration.AsMilliseconds()) >= 0;
        }
    }
    */
    return false;
