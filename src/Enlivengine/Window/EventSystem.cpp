...
bool EventSystem::EventAxis::Edit(ObjectEditor& objectEditor, const char* _name)
{
    if (objectEditor.BeginClass(_name, TypeInfo<EventSystem::EventAxis>::GetName(), TypeInfo<EventSystem::EventAxis>::GetHash()))
    {
        ...
        if (type == EventSystem::EventAxis::Type::JoystickAxis || type == EventSystem::EventAxis::Type::JoystickBallX || type == EventSystem::EventAxis::Type::JoystickBallY)
        {
...
void EventSystem::Update(Window& window)
{
    ...
    for (EventButton& eventButton : eventSystem.mButtons)
    {
        ...
        case EventButton::Type::Touch:
        {
            ...
            case EventButton::ActionType::Hold:
            {
                eventButton.active = Touch::IsHold(eventButton.buttonIdentifier);
                break;
            }
            case EventButton::ActionType::Pressed:
            {
                eventButton.active = Touch::IsPressed(eventButton.buttonIdentifier);
                break;
            }
            case EventButton::ActionType::Released:
            {
                eventButton.active = Touch::IsReleased(eventButton.buttonIdentifier);
                break;
            }
            default:
                enAssert(false);
...
U32 EventSystem::AddKeyButton(const char* name, Keyboard::Key key, EventButton::ActionType action, U32 modifiers)
{
...
U32 EventSystem::AddTouch(const char* name, U32 finger, EventButton::ActionType action)
{
    return AddButton(name, EventButton::Type::Touch, finger, 0, action);
}
...
void EventSystem::AddEventDebug(const std::string& eventDebug)
{
    auto& system = GetInstance();
    static constexpr std::size_t eventDebugSize = 30;
    system.mEventsDebug.push_back(eventDebug);
    if (system.mEventsDebug.size() > eventDebugSize)
    {
        system.mEventsDebug.erase(system.mEventsDebug.begin());
    }
}
...
void EventSystem::EventLoop(Window& window)
{
    sf::RenderWindow& sfmlWindow = window.GetSFMLWindow();
    ...
    while (sfmlWindow.pollEvent(event))
    {
        ...
        case sf::Event::TouchMoved:
            Touch::HandleEvent(event);
            break;
        case sf::Event::Closed:
            window.Close();
            break;
        case sf::Event::Resized:
            window.OnResized(&window, static_cast<U32>(event.size.width), static_cast<U32>(event.size.height));
            break;
        default:
            break;
        }
